  ...
  const handleDetailClick = async (id) => {
    try {
      ...
      const response = await axios.get(
        `https://devel4-filkom.ub.ac.id/bank-sampah/user/${id}`,
        { headers }
      );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://devel4-filkom.ub.ac.id/bank-sampah/user/${item.id}`,
        updatedFormData,
        { headers }
      );
  ...
  return (
    <>
      <button
        ...
        data-toggle="modal"
        data-target={`#modal_edit_nasabah_${item.id}`}
      ...
      <div
        ...
        data-dismiss="modal"
        ref={modalRef}
        id={`modal_edit_nasabah_${item.id}`}
